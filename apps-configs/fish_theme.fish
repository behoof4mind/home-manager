...
function show_host -d "Show host & user name"
    # Grab current kubectl context (part after oidc@)
    set context (kubectl config get-contexts --no-headers | grep -P '^\*' | awk '{print $2}' | grep -oP '(?<=oidc@).*')

    if test "$THEME_EDEN_SHOW_HOST" = yes
        # Default base color: red for root, blue for normal user
        if test (id -u) = 0
            set base_color (set_color red)
        else
            set base_color (set_color blue)
        end

        # Override color based on context name
        if string match -q -i '*prod*' "$context"
            set kube_color (set_color red)
        else if string match -q -i '*staging*' "$context"
            set kube_color (set_color yellow)
        else if string match -q -i '*qa*' "$context"
            set kube_color (set_color yellow)
        else
            set kube_color $base_color
        end

        # Kubernetes icon (Nerd Font)
        set k8s_icon " \u2388" # ﴱ

        # Print icon + context with chosen color
        echo -n "$kube_color*$context "(set_color normal)
    end
end
